...
const createVarName = (fileName) => (dashToPascalCase(fileName
    .toLowerCase()
    .replace(/[|&;$%@"<>()+,.{}_]/g, '-')));
...
const appDataPlugin = (config, compilerCtx, build, platform) => {
    if (!platform) {
        return {
            name: 'appDataPlugin',
        };
...
const getGlobalScriptPaths = (config, compilerCtx) => {
    const globalPaths = [];
...
const appendGlobalScripts = (globalPaths, s) => {
...
const getComponents = async (config, compilerCtx, buildCtx) => {
...
const visitClass = (config, diagnostics, typeChecker, classNode) => {
...
const validatePackageJson = async (config, compilerCtx, buildCtx) => {
    if (config.watch) {
        return;
    }
...
const validateBrowser = (compilerCtx, buildCtx) => {
...
const getRecommendedTypesPath = (config, outputTarget) => {
...
const packageJsonError = (compilerCtx, buildCtx, msg, warnKey) => {
...
const packageJsonWarn = (compilerCtx, buildCtx, msg, warnKey) => {
